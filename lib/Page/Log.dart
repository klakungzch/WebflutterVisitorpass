  ...
  Future<bool> getDataYearChart() async {
    await users.get().then((QuerySnapshot querySnapshot) => {
      querySnapshot.docs.forEach((doc) {
        List<String> dataSplit1checkin = doc.data()['checkIn_time'].split('/');
        ...
        List<String> dataSplit1checkout = doc.data()['checkOut_time'].split('/');
  ...
  Future<int> getDataCheck(String check) async {
    ...
    await users.get().then((QuerySnapshot querySnapshot) => {
      if(check == 'All'){
        ...
        querySnapshot.docs.forEach((doc) {
          List<String> dataSplit1 = doc.data()['checkIn_time'].split('/');
        ...
        querySnapshot.docs.forEach((doc) {
          if(doc.data()['status'].toString()=='CheckedOut'){
            List<String> dataSplit1 = doc.data()['checkOut_time'].split('/');
            List<String> dataSplit2 = dataSplit1[2].split(' ');
            String fullDate = dataSplit1[0]+'/'+dataSplit1[1]+'/'+dataSplit2[0];
            if(dataSearch != null)
              if(fullDate == dataSearch)
